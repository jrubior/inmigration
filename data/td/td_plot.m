    ...
    switch type
        case {0,1}
            ...
            plot(res.r,res.loglik,'ro',res.r,res.loglik);
                ...
                axis([min(res.r) max(res.r) min(res.loglik) max(res.loglik)])
                grid on;
...
grid off; legend ('y','y-sigma','y+sigma','Location','best');
...
t=1:n;
plot(t,y,'r-',t,xb,'b-');
grid off; legend ('y','x*beta','Location','best');
...
if (p == 2)
   ...
   t=1:n;
   plot(t,ty,'b-*',t,tx,'r-');
   grid off; legend ('y','x','Location','best');
   ...
   grid off; legend ('y','y+sigma','y-sigma','x','Location','best');
...
grid off; legend (meth,'y naive','Location','best');
...
grid off; legend ('u','U','Location','best');
title ('High frequency and low frequency residuals');
xlabel ('time'); 

% ACF and PACF of low-frequency residuals (requires LeSage's Econometric
% Toolbox
% figure;
