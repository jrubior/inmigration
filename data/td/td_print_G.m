function [] = td_print_G(res,file_sal)
...
if (p == 2)
   ...
   TY=rate(Y,1); TY=TY(2:N);
   TX=rate(X,1); TX=TX(2:N);
   AUX=corrcoef(TY,TX);
   c2=AUX(1,2);
   % Correlation between y and x (levels)
   ...
   ty=rate(res.y,sc);
   tx=rate(res.x(:,2),sc);
   AUX=corrcoef(ty(sc+1:n),tx(sc+1:n));
   c4=AUX(1,2);
   c5=std(ty(sc+1:n),1);
   c6=std(tx(sc+1:n),1);
end
...
ty=rate(res.y,sc);
txb=rate(xb,sc);
AUX=corrcoef(ty(sc+1:n),txb(sc+1:n));
c8=AUX(1,2);
