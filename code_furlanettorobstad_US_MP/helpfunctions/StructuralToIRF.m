function y = StructuralToIRF(x,setup)
%
%  model:
%
...
%    A0 = A(0)
%    Aplus = [A(1); ... A(p); C] 
%    Aplus = [ C; A(1); ... A(p);] % modified by JA Feb 17 2022
%    x = [vec(A0); vec(Aplus)]
%
% 
...
for i=1:p
    A{i}=Aplus(setup.nex+(i-1)*n+1:setup.nex+i*n,:);
    X = A0\A{i};
    for j=1:i-1
        X = X + L{j}*A{i-j};
    end
...
y((p+1)*n2+1:(p+1)*n2+nk)=reshape(Aplus(1:setup.nex,:),nk,1);
