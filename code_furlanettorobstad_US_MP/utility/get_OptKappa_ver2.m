...
% This version maximizes w.r.t. kappa1, kappa2 and kappa3
%
% See:
...
function [ml_opt,kappa_opt] = get_OptKappa_ver2(Y0,Y,Z,p,k0,type,idx_ns)
if nargin == 6
    idx_ns = [];
end
kappa4 = 100;
...
if strcmp(type,'stru')
    f = @(k) -ml_VAR_ACP(p,Y,Z,prior_ACP_stru(n,p,[exp(k(1)),exp(k(2)),exp(k(3)),kappa4],sig2,idx_ns));
elseif strcmp(type,'redu')
    f = @(k) -ml_VAR_ACP(p,Y,Z,prior_ACP_redu(n,p,[exp(k(1)),exp(k(2)),exp(k(3)),kappa4],sig2,idx_ns));
end
[k_opt,nml] = fminsearch(f,log(k0));
ml_opt = -nml;
kappa_opt = [exp(k_opt(1)),exp(k_opt(2)),exp(k_opt(3)),kappa4];
end